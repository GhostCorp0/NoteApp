...
import com.ghostcorp.notesapp.feature_note.domain.util.NoteOrder
import com.ghostcorp.notesapp.feature_note.domain.util.OrderType
...
class NotesViewModel @Inject constructor(private val noteUseCases: NoteUseCases) : ViewModel() {
    ...
    fun onEvent(event: NotesEvent) {
        when (event) {
            is NotesEvent.Order -> {
                if(state.value.noteOrder::class == event.noteOrder::class && state.value.noteOrder.orderType == event.noteOrder) {
                    return
                }
